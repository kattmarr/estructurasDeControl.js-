...
function cuentaRegresiva(numero) {
    ...
    while (numero >= 0) {
        console.log(numero);
        numero--; // Decrementamos el número en cada iteración
    }
}
...
function detenerEnSeis() {
    ...
    for (let i = 1; i <= 10; i++) {
        if (i === 6) {
            break; // Detenemos el bucle cuando el número es igual a 6
        }
        console.log(i);
    }
}
...
function esVocal(letra) {
    // Tu código aquí
    switch (letra.toLowerCase()) { // Convertimos la letra a minúscula para evitar errores con mayúsculas
        case 'a':
        case 'e':
        case 'i':
        case 'o':
